...
<!--    main content section-->
   <div class="row">
		...
		$number=$_POST['number'];
		$query = $pdo->prepare("select * from train_data INNER JOIN topic_data ON train_data.topic_id = topic_data.id INNER JOIN group_data ON train_data.group_id = group_data.id where train_data.phone_no LIKE '%$number%'  LIMIT 0 , 100");
		$query->bindValue(1, "%$number%", PDO::PARAM_STR);
		$query->execute();
